---
title: "A Small Retrospective on my Giza Project"
date: 2021-01-12
...
- Handling missing data of all types throughout code, nonetypes throw errors a lot!
- How painful large file storage is. Especially if you do not want to pay for it. Smaller data is always preferable.
- Session and retry protocol for http requests to improve performance and reduce errors.
- Asynchronous http requests with Grequests to overcome IO bound.
- Source website pushed buggy updates mid-development. Lots of real-world error handling experience. Website changed critical elements to javascript rendering. Switched Selenium.
- LXML parser speed improvements for Beautifulsoup performance improvements.
- Multithreaded computation with Pandarallel for CPU bound performance improvements.
...
The unstructured nature makes a simple regex solution impossible. A sweet future improvement would be to use an ML model to identify and extract pitch details even as the format varies wildly.
...
It is not consistent, but often-times a letter grade mix such as 5.11c/d is used to say that it is either a tough 5.11c or easy 5.11d. This is comparable to 5.11+, which may mean the same thing, or that it is a tough 5.11d depending on the mood of whomever claimed the grade.

If we wanted to homegenize between the three variations, it is easy to assign the basic letter grade to a sign type. For example a 5.10a is a 5.10-, a 5.10b and 5.10c are 5.10, and a 5.10d is a 5.10+. This leads to imbalanced binning and a reduction of resolution. This is what MP does.
...
Consider the following grade conversion chart, and consider that this does not even consider the subtler variants!
...
# Scraping
I prototyped with Requests. It is simple and easy to use as they say. It became apparent that I was IO bound on this task, as a scrape of ~1000 routes could take upwards of 45 minutes. I switched to Grequests for asyncronous scraping. Using 100 agents reduced the scrape time by about 80%, awesome! Or at least it was, until I integrated the scraping script into my streamlit script. The issue is that streamlit itself uses asyncronous protocols, and if you've ever used async code, it is finnicky on ensuring the async expressions are consistent across function calls etc. Since I was not about to dive into the inner layer code of streamlit, I accepted that my users might just have to be patient, bummer.

This ended up being a moot point since as I was deliberating this, MP updated their website to use javascript to load in content. This required a change to Selenium in order to wait for the content to load, then repeatedly click a "load more" button so I could have all of the available data. As such, my scrape got even slower by ~20%. One of the more difficult setup issues this presented was figuring out how to reliably install the required web driver on the Linux distro that the streamlit cloud instance was hosted on. This was a good lesson on the classic merits of developing on the same OS you will be deploying to. I found a premade docker image that is intended for developing streamlit cloud apps. Luckily, I was able to get things running with a few smart github commits, and never needed to spin it up. Usually a good idea to at least try your luck at the lazy way once or twice.
...
These computations ended up making me CPU bound. I was able to improve performance by ~20% using Pandarallel parallel threading. This ended up dying the same fate that async did, as it just did not play well with Streamlit.
...
* Multiple options for sub-type partitioning key charts.
...
# Conclusion
Overall I'm really pleased with how this project turned out. I think I'm most proud how I was able to move beyond pre-constructed analysis on a pre-selected dataset, and allow analysis of any arbitrary MP dataset. Myself and friends have used my tool many times already to identify neat accomplishments and find cool new climbs. I also look forward to more specific analysis I can run using this code as a launch point.
